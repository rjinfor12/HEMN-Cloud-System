    ...
    def _run_enrich(self, tid, input_file, output_dir, name_col, cpf_col):
        ...
        try:
            ...
            if search_terms:
                ...
                print(f"DEBUG: basic_list size: {len(basic_list)}")
                print(f"DEBUG: basic_list size: {len(basic_list)}")
                if basic_list:
                    ...
                    for hit in all_details:
                        ...
                        if len(global_cache) == 0: print(f"DEBUG: First hit in global_cache: {cpf_mask}")
            ...
            if global_cache:
                ...
                print(f"DEBUG: Merged found count: {found_count}")
                print(f"DEBUG: Merged found count: {found_count}")
            ...
            print(f"DEBUG: Attempting to save to: {output_file}")
            print(f"DEBUG: Attempting to save to: {output_file}")
            df_final.to_excel(output_file, index=False)
            print(f"DEBUG: Save complete. Checking existence...")
            print(f"DEBUG: File exists on disk: {os.path.exists(output_file)}")
            print(f"DEBUG: Save successful: {os.path.exists(output_file)}")
