...
class CloudEngine:
    ...
    def get_user_tasks(self, username):
        ...
        rows = conn.execute(
            "SELECT * FROM background_tasks WHERE username = ? COLLATE NOCASE AND (status IN ('QUEUED', 'PROCESSING') OR (created_at > datetime('now','-24 hours'))) ORDER BY created_at DESC", 
            (username,)
        ).fetchall()
        conn.close()
