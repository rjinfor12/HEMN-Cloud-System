...
class CloudEngine:
    ...
    def _run_enrich(self, tid, input_file, output_dir, name_col, cpf_col):
        ...
        try:
            ...
            total = len(df_in)
            print(f"DEBUG: Input Rows: {total}")
            ...
            print(f"DEBUG: Search Terms: {len(search_terms)}")
            
            global_cache = {}
            found_count = 0
            
            if search_terms:
                self._update_task(tid, progress=5, message="Buscando Matrizes no ClickHouse...")
                ...
                # Fetch basic CNPJs for empresas (direct CNPJ matches for MEI)
                ...
                print(f"DEBUG: Basic List: {len(basic_list)}")
                if basic_list:
                    ...
                    print(f"DEBUG: Rows from ClickHouse: {len(all_details)}")
                    db_cols = res_details.column_names
                    
                    self._update_task(tid, progress=75, message="Filtrando Resultados por Prioridade...")
                    
                    mapping = {'01': 'NULA', '02': 'ATIVA', '03': 'SUSPENSA', '04': 'INAPTA', '08': 'BAIXADA'}
                    
                    for hit in all_details:
                        hit_dict = dict(zip(db_cols, hit))
                        cpf_mask = hit_dict['cpf_mask'] or hit_dict['cnpj_basico']
                        ...
                        global_cache[cpf_mask] = {
                            ...
                            'cnae': hit_dict['cnae_fiscal'], 
                            'logradouro': hit_dict['logradouro'], 
                            'numero': hit_dict['numero'], 
                            'complemento': hit_dict['complemento'],
                            'bairro': hit_dict['bairro'], 
                            'cidade': str(hit_dict['municipio_nome']).upper(), 
                            'uf': hit_dict['uf'], 
                            'cep': hit_dict['cep'],
                            'ddd_novo': hit_dict['ddd1'], 
                            'telefone_novo': hit_dict['telefone1'], 
                            'email_novo': hit_dict['correio_eletronico'],
            ...
            self._update_task(tid, progress=80, message="Estruturando resultados finais...")
            ...
            if global_cache:
                ...
                print(f"DEBUG: df_merged columns: {df_merged.columns.tolist()}")
                ...
                print(f"DEBUG: Found Count Merged: {found_count}")
                print(f"DEBUG: Attempting to drop columns from df_merged...")
                df_final = df_merged.drop(columns=['titanium_cpf', 'titanium_nome', 'mask_calc', 'lookup_key', 'CNPJ', 'SITUACAO_CODIGO'], errors='ignore')
                print(f"DEBUG: Drop successful. df_final columns: {df_final.columns.tolist()}")
            else:
                found_count = 0
                df_final = df_in.drop(columns=['titanium_cpf', 'titanium_nome'], errors='ignore')

            self._update_task(tid, progress=95, message="Salvando arquivo resultante...")
            print(f"ABS_OUTPUT_DIR: {os.path.abspath(output_dir)}")
            print(f"ABS_OUTPUT_FILE: {os.path.abspath(output_file)}")
            print(f"DEBUG: Saving to {output_file}...")
            print(f"BEFORE_SAVE_UPLOADS: {os.listdir(output_dir)}")
            df_final.to_excel(output_file, index=False)
            print(f"AFTER_SAVE_UPLOADS: {os.listdir(output_dir)}")
            print(f"VERIFY_EXISTS: {os.path.exists(output_file)}")
            print(f"DEBUG: Save complete. Exists: {os.path.exists(output_file)}")
            
            fc_native = int(found_count)
            total_native = int(total)
            msg = f"TITANIUM-DONE: Processou {total_native} linhas. {fc_native} encontrados."
    ...
    def _run_extraction(self, tid, filters, output_dir):
        ...
        try:
            ...
            q = f"SELECT e.razao_social, concat(estab.cnpj_basico, estab.cnpj_ordem, estab.cnpj_dv) as CNPJ, m.descricao as CIDADE, estab.uf, estab.ddd1, estab.telefone1, estab.ddd2, estab.telefone2 FROM hemn.estabelecimento estab JOIN hemn.empresas e ON estab.cnpj_basico = e.cnpj_basico LEFT JOIN hemn.municipio m ON estab.municipio = m.codigo WHERE {where_clause} LIMIT 100000"
    ...
    def _run_carrier(self, tid, input_file, output_dir, phone_col):
        ...
        try:
            ...
            for i, row in enumerate(df.itertuples(index=False)):
                if i % 500 == 0: self._update_task(tid, progress=int((i/len(df))*100))
