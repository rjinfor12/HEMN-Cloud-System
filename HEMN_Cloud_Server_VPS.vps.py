...
async def change_self_password(request: Request, user: dict = Depends(get_current_user)):
    try: data = await request.json()
    except: data = await request.form()
    ...
    new_hash = get_password_hash(new_p)
    conn = sqlite3.connect(DB_PATH, timeout=30)
    conn.execute("PRAGMA journal_mode=WAL")
    try:
        conn.execute("UPDATE users SET password = ?, force_password_change = 0 WHERE username = ?", (new_hash, user["username"]))
        conn.commit()
    except Exception as e:
        conn.close()
        raise HTTPException(status_code=500, detail=str(e))
...
@app.post("/admin/monitor/update-carrier")
def update_carrier(user: dict = Depends(get_current_user)):
    if user["role"] != "ADMIN": 
        raise HTTPException(status_code=403, detail="Acesso negado")
    return engine.start_carrier_update(username=user["username"])
...
def reset_password(username: str, user: dict = Depends(get_current_user)):
    if user["role"] != "ADMIN": 
        raise HTTPException(status_code=403, detail="Acesso restrito a administradores.")
    ...
    try:
        new_hash = get_password_hash("hemn123")
        conn.execute("UPDATE users SET password = ?, force_password_change = 1 WHERE username = ? COLLATE NOCASE", (new_hash, username))
        conn.commit()
    except Exception as e:
        conn.close()
        raise HTTPException(status_code=500, detail=str(e))
