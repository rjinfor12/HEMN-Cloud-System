...
# Tentando importar CloudEngine do cloud_engine.py
try:
    from cloud_engine import CloudEngine
    engine = CloudEngine()
except Exception as e:
    print(f"FAILED TO IMPORT CloudEngine: {e}")
    sys.exit(1)
...
try:
    ...
    # Simula a normalização agressiva que adicionei por último
    df.columns = [str(c).upper().strip().replace('_', ' ') for c in df.columns]
    print(f"COLUMNS AFTER NORMALIZATION (UPPER + SPACE): {df.columns.tolist()}")
    ...
    # O Ponto Crítico: Onde os dados se perdem?
    for c in final_columns:
        if c not in df.columns:
            print(f"WARNING: column '{c}' NOT FOUND in df. Setting to blank.")
            df[c] = ""
            
    df = df[final_columns]
    print("--- FIRST ROW PRE-EXPORT ---")
...
print('=== EXECUTANDO DIAGNÓSTICO PROFUNDO (V2) NA VPS ===')
client.exec_command("cat << 'EOF' > /tmp/deep_diag_blank_v2.py\n" + diag_script + "\nEOF")
stdin, stdout, stderr = client.exec_command("/var/www/hemn_cloud/venv/bin/python3 /tmp/deep_diag_blank_v2.py")
