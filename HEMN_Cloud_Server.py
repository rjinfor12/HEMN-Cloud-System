...
async def read_monitor():
    return FileResponse(
        os.path.join(APP_DIR, "admin_monitor_vps.html"),
...
def verify_password(plain_password, hashed_password):
    try:
        # PBKDF2 handles longer passwords natively and avoids the 72-char limit
...
async def login(request: Request):
    ...
    if user:
        # Check if password matches (securely)
        is_valid = False
...
async def change_password(req: PasswordChangeRequest, user: dict = Depends(get_current_user)):
    ...
    db_user = conn.execute("SELECT * FROM users WHERE username = ? COLLATE NOCASE", (user["username"],)).fetchone()
    if not db_user:
        conn.close()
    ...
    # Update password (hash it) and clear force_password_change flag
    new_hash = get_password_hash(req.new_password)
    conn.execute("UPDATE users SET password = ?, force_password_change = 0 WHERE username = ? COLLATE NOCASE", (new_hash, user["username"]))
    conn.commit()
    conn.close()
...
async def get_me(user: dict = Depends(get_current_user)):
    ...
    return {**user, "system_version": SYSTEM_VERSION}
